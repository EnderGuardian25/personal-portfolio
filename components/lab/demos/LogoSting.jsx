...
export default function LogoSting({ reducedMotion }) {
  ...
  useEffect(() => {
    ...
    strokes.forEach((p, i) => gsap.set(p, { strokeDasharray: lens[i] }));

    const center = (r) => [r.left + r.width / 2, r.top + r.height / 2];
    const tl = gsap.timeline({ delay: 0.35 });
    tl.set(header, { autoAlpha: 0 })
      .set(logo, { x: 0, y: 0, scale: 1, autoAlpha: 1 })
      // 1 — strokes draw in sequence: stem, bowl, counter
      .fromTo(strokes[0], { strokeDashoffset: lens[0] }, { strokeDashoffset: 0, duration: 0.5, ease: "power2.inOut" })
      .fromTo(strokes[1], { strokeDashoffset: lens[1] }, { strokeDashoffset: 0, duration: 0.7, ease: "power3.inOut" }, "-=0.22")
      .fromTo(strokes[2], { strokeDashoffset: lens[2] }, { strokeDashoffset: 0, duration: 0.45, ease: "power2.inOut" }, "-=0.3")
      // 2 — fill wipes up under the strokes
      .fromTo(fillWrap, { clipPath: "inset(100% 0% 0% 0%)" }, { clipPath: "inset(0% 0% 0% 0%)", duration: 0.38, ease: "power4.inOut" }, "-=0.1")
      // 3 — pop with overshoot + one accent flash
      .fromTo(flash, { autoAlpha: 0 }, { autoAlpha: 0.85, duration: 0.07, ease: "power1.in" })
      ...
      .fromTo(bits, { autoAlpha: 0, y: 16 }, { autoAlpha: 1, y: 0, duration: 0.6, stagger: 0.08, ease: "power3.out" }, "<");

    tlRef.current = tl;
    return () => {
      tl.kill();
      // drop gsap's inline styles so a reducedMotion flip shows the settled scene
      gsap.set([header, logo, ...bits], { clearProps: "all" });
      tlRef.current = null;
    };
  }, [reducedMotion]);
    ...
    <div ref={rootRef} className="relative h-full w-full overflow-hidden bg-lab-bg">
      ...
      {!reducedMotion && (
        <button
          ...
          onClick={() => tlRef.current?.invalidate().restart()}
