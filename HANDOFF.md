...
- [ ] **Commit + deploy the 2026-07-18 performance pass** (EagerReveal, SSR IntroStamp, GA lazyOnload — working tree, uncommitted) then SSH into VPS → `~/deploy.sh`; re-run Lighthouse against the LIVE site in Incognito (extension-free) — expect 100 desktop on both pages, mobile limited mainly by fonts + TTFB
- [ ] **/services not indexed by Google** — technical signals verified clean 2026-07-18 (in sitemap, canonical OK, no noindex/X-Robots-Tag, robots.txt allows). Remaining causes: GSC sitemap submission + "Request indexing" for `/services` never done (below), young low-authority domain, `www.` serves 200 without redirecting to apex (canonical covers it, but a 301 would consolidate). Check URL Inspection in GSC for the verdict string
- [ ] **Deploy** — SSH into VPS → `~/deploy.sh` (deploys main — includes lab wave 3 `e2ded2d` + liquid-type rework `3baa9fd`, both pushed 2026-07-13 but NOT yet deployed)
...
- [ ] `next build` regression check before deploy (wave 3 wasn't build-tested — dev-verified only; expect 81 static pages)
